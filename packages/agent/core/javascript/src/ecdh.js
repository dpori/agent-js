  ...
  var ECDH = {
    ...
    KEY_PAIR_GENERATE: function (RNG, S, W) {
      ...
      if (RNG === null) {
        ...
        if (ctx.ECP.CURVETYPE != ctx.ECP.WEIERSTRASS) s = ctx.BIG.random(RNG);
        // from random bytes
        else s = ctx.BIG.randomnum(r, RNG); // Removes biases
      }

      if (ctx.ECP.CURVETYPE != ctx.ECP.WEIERSTRASS) this.RFC7748(s); // For Montgomery or Edwards, apply RFC7748 transformation
